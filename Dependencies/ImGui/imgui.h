...
namespace ImGui
{
	...
	IMGUI_API void          TextNoFmt(const char* text);												   // simple formatted text
	IMGUI_API void          TextVNoFmt(const char* text);
	...
	IMGUI_API void          TextWrappedNoFmt(const char* text);											   // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
	IMGUI_API void          TextWrappedVNoFmt(const char* text);
